...
# Cria o diretório pai 'MobileNetMNIST_' com incremento, se necessário
diretorio_pai = criar_diretorio_incrementado('resultadosMobileNNet', 'mobileNetMNIST')
...
tempos_treino = []
potencias_treino = []
tracker = CarbonTracker(epochs=maximo_epocas, components="gpu", monitor_epochs=-1, interpretable=True,
...
#             nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1, bias=False), # Camada inicial
#             nn.BatchNorm2d(32),
#             nn.ReLU(inplace=True),
#             conv_dw(32, 64, 1),  # Downsampling aqui
#             nn.Dropout2d(0.5),
#             conv_dw(64, 128, 1),  # Downsampling aqui
#             conv_dw(128, 128, 1), # Sem downsampling
#             nn.AdaptiveAvgPool2d(1), # Pooling adaptativo para garantir saída 1x1
#             nn.Flatten(),
#             nn.Linear(128, 10)
#         )
#
    ...
    def __init__(self):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
             ...
             nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, groups=64, bias=False), # Downsampling
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
...
def treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, epocas, early_stopping):
    modelo.train()
...
for i in range(numero_modelos):
    ...
    perda_treino, acuracia_treino, perda_validacao, acuracia_validacao, tempo_treino, consumo_energia = (
        treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, maximo_epocas, early_stopping))
    metricas.append((perda_treino, acuracia_treino, perda_validacao, acuracia_validacao, tempo_treino.total_seconds(),
                     consumo_energia))
    ...
    print(
        f'Modelo {i + 1}: Média Perda Treino: {media_perda_treino:.4f}, Média Acurácia Treino: {media_acuracia_treino:.4f}, '
