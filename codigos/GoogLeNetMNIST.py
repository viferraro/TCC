...
# Cria o diretório pai 'GoogLeNetMNIST_' com incremento, se necessário
diretorio_pai = criar_diretorio_incrementado('resultadosGoogLeNet', 'GoogleNetMNIST')
...
contagem_classes = Counter([rotulo for _, rotulo in conjunto_treino])
print(f'Distribuição das classes no conjunto de treinamento: {contagem_classes}')
...
for i in range(numero_modelos):
    ...
    perda_treino, acuracia_treino, perda_validacao, acuracia_validacao, tempo_treino, consumo_energia = (
        treinar_e_validar(modelo, carregador_treino, carregador_validacao, criterio, otimizador, 50, early_stopping))
